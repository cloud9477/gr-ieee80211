...
# 2, 0, 2, 0, 0
tmpPacketNdpGr = b'\x02\x00\x02\x00\x00'
apMacSocket.sendto(tmpPacketNdpGr, (apPhyIp, apPhyPort))
        ...
        if(mac80211.procCheckCrc32(tmpPkt[:-4], tmpPkt[-4:])):
            ...
            if(tmpType == 136):
                ...
                if(staChanRcvFlag0 and staChanRcvFlag1):
                    ...
                    fig_ax_lineR.set_ydata(np.real([each[0][0] for each in bfQForFftNormdForFft]))
                    fig_ax_lineI.set_ydata(np.imag([each[0][0] for each in bfQForFftNormdForFft]))
                    fig_bx_lineR.set_ydata(np.real([each[0][1] for each in bfQForFftNormdForFft]))
                    fig_bx_lineI.set_ydata(np.imag([each[0][1] for each in bfQForFftNormdForFft]))
                    fig_cx_lineR.set_ydata(np.real([each[1][0] for each in bfQForFftNormdForFft]))
                    fig_cx_lineI.set_ydata(np.imag([each[1][0] for each in bfQForFftNormdForFft]))
                    fig_dx_lineR.set_ydata(np.real([each[1][1] for each in bfQForFftNormdForFft]))
                    fig_dx_lineI.set_ydata(np.imag([each[1][1] for each in bfQForFftNormdForFft]))
                    # refresh figure
