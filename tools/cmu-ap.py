...
if __name__ == "__main__":
    ...
    compChan0 = []
    compChan1 = []
    fWaveComp = open("/home/cloud/sdr/cmu-chan0.bin", 'rb')
    for i in range(0,128):
        ...
        compChan0.append(tmpR + tmpI * 1j)
    fWaveComp.close()
    fWaveComp = open("/home/cloud/sdr/cmu-chan1.bin", 'rb')
    for i in range(0, 128):
        tmpR = struct.unpack('f', fWaveComp.read(1) + fWaveComp.read(1) + fWaveComp.read(1) + fWaveComp.read(1))[0]
        tmpI = struct.unpack('f', fWaveComp.read(1) + fWaveComp.read(1) + fWaveComp.read(1) + fWaveComp.read(1))[0]
        compChan1.append(tmpR + tmpI * 1j)
    fWaveComp.close()
    ...
    nRx = 1
    ltfSym = []
    ltfSym.append(p8h.procRemovePilots(p8h.procFftDemod(compChan0[0:64], nScDataPilot, nSts)))
    ltfSym.append(p8h.procRemovePilots(p8h.procFftDemod(compChan0[64:128], nScDataPilot, nSts)))
    # compute feedback
    ...
    for each in vFb1:
        print(each)
    ltfSym = []
    ltfSym.append(p8h.procRemovePilots(p8h.procFftDemod(compChan1[0:64], nScDataPilot, nSts)))
    ltfSym.append(p8h.procRemovePilots(p8h.procFftDemod(compChan1[64:128], nScDataPilot, nSts)))
    # compute feedback
